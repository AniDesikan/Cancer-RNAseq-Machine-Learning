...
load_n_trim <- function(filename) {
    file <- as.data.frame(read_tsv(filename)) 
    trimmed <-  dplyr::select(file, vP0_1, vP0_2, vAd_1, vAd_2)
    row.names(trimmed) <- file[,"gene"]
    return(trimmed)
}
...
run_deseq <- function(count_dataframe, coldata, count_filter, condition_name) {
  ...
  filtered_counts <- count_dataframe[rowSums(count_dataframe) >=count_filter,]
...
run_edger <- function(count_dataframe, group) {
  # Create a DGEList object from count_dataframe
...
run_limma <- function(counts_dataframe, design, group) {
  ...
  return(topTable(fit,coef=ncol(design), sort.by = "p", number = 15026))
}
...
combine_pval <- function(deseq, edger, limma) {
  deseq_pval <- deseq$pvalue
  edger_pval <- edger$PValue
  limma_pval <- limma$P.Value
  pval_data <- data.frame(
    package = c(rep("DESeq2", length(deseq_pval)),
                rep("edgeR", length(edger_pval)),
                rep("Limma", length(limma_pval))),
    pval = c(deseq_pval, edger_pval, limma_pval)
  )
  
  # Return the resulting tibble or dataframe
  return(pval_data)
}
...
create_facets <- function(deseq, edger, limma) {
  # Create separate data frames for each package
  deseq_df <- tibble(package = "deseq",logFC = deseq$log2FoldChange, padj = deseq$padj)
  edger_df <- tibble(package = "edgeR",logFC = edger$logFC, padj = edger$padj)
  limma_df <- tibble(package = "limma",logFC = limma$coef, padj = limma$adj.P.val)
  
  combined_df <- bind_rows(deseq_df, edger_df, limma_df)
