...
meta <- meta_info_from_labels(colnames(count_data)[colnames(count_data)!='gene'])
meta
```
...
library
ggplot(library, mapping = aes(x=sample,y=value, fill=sample)) +
...
DESeq2_tibble <- deseq_normalize(count_data, meta)
DESeq2_tibble
...
plot_sample_distributions(count_data, scale_y_axis=TRUE, title='Raw Count Distributions')
...
var_mean <- plot_variance_vs_mean(count_data, scale_y_axis=TRUE, title='variance vs mean (raw counts)')
var_mean
...
pca_counts <- plot_pca(counts_data meta, "Raw Count PCA")
pca_counts
```
...
cpm_dist <- plot_sample_distributions(cpm, scale_y_axis=TRUE, title='Raw Count Distributions')
cpm_v_m <- plot_variance_vs_mean(cpm, scale_y_axis=TRUE, title='variance vs mean (raw counts)')
cpm_pca_counts <- plot_pca(cpm, meta, "Raw Count PCA")
cpm_dist
cpm_v_m
cpm_pca_counts
```
...
DE_dist <- plot_sample_distributions(DESeq2_tibble, scale_y_axis=TRUE, title='Raw Count Distributions')
DE_v_m <- plot_variance_vs_mean(DESeq2_tibble, scale_y_axis=TRUE, title='variance vs mean (raw counts)')
DE_pca_counts <- plot_pca(DESeq2_tibble, meta, "Raw Count PCA")
DE_dist
DE_v_m
DE_pca_counts
```